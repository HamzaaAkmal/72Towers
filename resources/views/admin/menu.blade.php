        ...
        <ul class="codex-menu custom-scroll" data-simplebar>
            ...
            @if (Gate::check('manage property') ||
                ...
                @if (Gate::check('manage client') || Gate::check('manage property') || Gate::check('manage unit'))
                    <li
                        ...
                        <ul class="submenu-list"
                            ...
                            @if (Gate::check('manage property'))
                                <li
                                    class=" {{ in_array($routeName, ['property.index', 'property.create', 'property.edit', 'property.show']) ? 'active' : '' }} ">
                ...
                @if (Gate::check('manage maintainer') || Gate::check('manage maintenance request'))
                    <li
                        class="menu-item {{ in_array($routeName, ['maintenance-request.index', 'maintenance-request.pending', 'maintenance-request.inprogress']) ? 'active' : '' }}">
                        <a href="javascript:void(0);">
                        ...
                        <ul class="submenu-list"
                            ...
                            @if (Gate::check('manage maintenance request'))
                                <li class=" {{ in_array($routeName, ['maintenance-request.inprogress']) ? 'active' : '' }}">
                                    <a href="{{ route('maintenance-request.inprogress') }}">{{ __('In Progress') }}
                                    </a>
                                </li>
                            @endif
                ...
                @if (Gate::check('manage invoice') || Gate::check('manage expense'))
                    <li
                        class="menu-item {{ in_array($routeName, ['invoice.index', 'invoice.create', 'invoice.edit', 'invoice.show', 'expense.index']) ? 'active' : '' }}">
                        <a href="javascript:void(0);">
                        ...
                        <ul class="submenu-list"
                            ...
                            @if (Gate::check('manage invoice'))
                                <li
                                    class=" {{ in_array($routeName, ['invoice.index', 'invoice.create', 'invoice.edit', 'invoice.show']) ? 'active' : '' }}">
            ...
            @if (Gate::check('manage types') || Gate::check('manage notification'))
                <li class="cdxmenu-title">
                    <h5>{{ __('Setup') }}</h5>
                </li>
                ...
                @if (Gate::check('manage notification'))
                    <li class="menu-item {{ in_array($routeName, ['notification.index']) ? 'active' : '' }}">

...
<a class="btn btn-primary btn-sm ml-20 customModal" href="#" data-size="lg"
   data-url="{{ route('unit.direct-create') }}"
   data-title="{{__('Create Unit')}}"> <i class="ti-plus mr-5"></i>{{__('Create unit')}}</a>
@endcan
@endsection
@section('content')
                ...
                <div class="card-body">
                    <table class="display dataTable cell-border datatbl-advance">
                        ...
                        @foreach($units as $unit)
                            <tr role="row">
                                <td>{{$unit->name}} </td>
                                <td>{{$unit->charge_type}} </td>
                                <td>{{priceFormat($unit->amount_due)}} </td>
                                <td>
                                    @if($unit->charge_type=='custom')
                                    ...
                                    @else
                                        {{$unit->charge_duration}}
                                    @endif
                                </td>
                                <td>{{!empty($unit->properties)?$unit->properties->name:'-'}} </td>
                                <td>{{!empty($unit->clients()) && !empty($unit->clients()->user)?$unit->clients()->user->name:'-'}} </td>
                                @if(Gate::check('edit unit') || Gate::check('delete unit'))
                                    <td class="text-right">
